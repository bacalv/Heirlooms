...
sequenceDiagram
    ...
    S-->>S: markInviteUsed<br/> createFriendship(inviter, newUser)
...
sequenceDiagram
    ...
    Note over U,C: User already has device key<br/> no auth_verifier set yet
...
sequenceDiagram
    ...
    S-->>S: generate 8-digit numeric code<br/> store pairing record
    S->>A: 200 {one_time_code, expires_at}

    Note over A,W: User reads code on Android screen<br/> types into web app

    W->>S: POST /pairing/qr {code}
    S-->>S: validate code -> create session_id<br/> store in pairing record
    S->>W: 200 {session_id}
    ...
    S-->>S: issueToken for web session<br/> store wrapped_master_key
    S->>A: 200 {ok: true}
    ...
    Note over W,C: Master key now in memory<br/> web session established
```
...
sequenceDiagram
    ...
    S-->>S: generate code + linkId<br/> state = "initiated"
    S->>T: 200 {linkId, code}
    ...
    S-->>S: state -> "device_registered"<br/> store new device pubkey
    S->>N: 202 Accepted
    ...
    S-->>S: insertWrappedKey for new device<br/> state -> "wrap_complete"
    ...
    Note over N,C: Master key in memory<br/> new device fully set up
```
...
sequenceDiagram
    ...
    S-->>S: generate 32-byte random token<br/> store invite (expires 48h)
