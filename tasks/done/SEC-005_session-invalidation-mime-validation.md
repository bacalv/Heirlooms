...
**F-07 — Session invalidation on passphrase change:**
- Added `deleteAllSessionsForUser(userId: UUID)` to the `AuthRepository` interface and implemented it in `PostgresAuthRepository` with `DELETE FROM user_sessions WHERE user_id = ?`.
- In `AuthService.setupExisting()`, called `authRepo.deleteAllSessionsForUser(user.id)` immediately after `setUserAuth` and before issuing the new session token, ensuring all prior sessions are invalidated.
- Added integration test `setup-existing invalidates prior sessions — old token returns 401` in `AuthHandlerTest.kt`.

**F-08 — MIME type allowlist:**
- Created `HeirloomsServer/src/main/kotlin/digital/heirlooms/server/routes/upload/UploadValidation.kt` with `ALLOWED_MIME_TYPE_PATTERNS` (Set of Regex for `^image/.+` and `^video/.+`) and `isAllowedMimeType(mimeType)` helper.
- In `UploadRoutes.kt` `initiateUploadContractRoute`, added check before calling `uploadService.initiateUpload`: if `mimeType` is non-blank and not allowed, return `400 Bad Request` with body `"Unsupported media type"`.
- Added tests in `UploadHandlerTest.kt`: `application/pdf`, `audio/mpeg`, `application/octet-stream` all return 400; `image/heic` is accepted.
