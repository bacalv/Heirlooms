...
- Wrote `docs/briefs/ARCH-006_tlock-provider-interface.md`.
- Amended `docs/briefs/ARCH-003_m11-capsule-crypto-brief.md` with §9 blinding scheme
  addendum and schema updates.
- Chose Option A (dual ECDH fields) to resolve the iOS/blinding path contradiction:
  `wrapped_capsule_key` always wraps `DEK` (iOS compatibility); new
  `wrapped_blinding_mask` wraps `DEK_client` for Android/web blinded path.
- Clarified that Shamir shares are computed over `DEK`, not `DEK_client`.
- Added `tlock_dek_tlock` column (stores `DEK XOR DEK_client` server-side;
  served via `/tlock-key`).
- Added `tlock_key_digest = SHA-256(DEK_tlock)` column for tamper detection.
- Documented `/tlock-key` response body logging prohibition in both briefs.
- Recommended sidecar (Option B) for M12 real drand integration; specified full
  REST API contract for the Go sidecar.
- `StubTimeLockProvider` uses a 64-byte blob (plaintext + HMAC-SHA256 MAC)
  and simple time-check gate; no real BLS12-381 required for staging.
