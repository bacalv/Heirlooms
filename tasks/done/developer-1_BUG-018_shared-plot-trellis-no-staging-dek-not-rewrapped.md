...
Completed by developer-1 on 2026-05-15.

### Changes made

**`TrellisRecord.kt`** — Added `targetPlotVisibility: String = ""` field (default empty
for backward compat with mocked tests).

**`TrellisRepository.kt`**:
- `createTrellis`: changed staging policy `when` branch to `"public", "shared" -> true`
- `updateTrellis`: same change applied
- `listTrellises` / `getTrellisById`: updated SQL to `LEFT JOIN plots p ON p.id = t.target_plot_id`
  and select `COALESCE(p.visibility, '') AS target_plot_visibility`
- `toTrellisRecord()`: reads `target_plot_visibility` from result set
- `runUnstagedTrellisesForUpload`: added `.filter { it.targetPlotVisibility != "shared" }`
  defensive guard after the existing `!requiresStaging` filter

**`V31__shared_plot_trellis_staging.sql`** (new migration): UPDATE existing trellises
targeting shared plots to set `requires_staging = true`.

**`FlowsPage.jsx`** — Added `targetIsShared` derived value in `FlowForm`. When
`targetIsShared` is true, the auto-approve checkbox is replaced with an informational
note explaining staging is always required for shared plots.

**`TrellisServiceTest.kt`** — Added BUG-018 test verifying the service calls through
to the repository when `requiresStaging=false` is passed for a shared plot (repo
enforces the override).

**`SchemaMigrationTest.kt`** — Added `V31 migration SQL fixes shared-plot trellis
with requires_staging false` canary test that inserts a legacy-style row and verifies
the V31 UPDATE SQL correctly flips it to true.

### Tests

All tests pass: `./gradlew test --no-daemon` — BUILD SUCCESSFUL (26s).
