...
///
/// Matches the upload logic in `HomeView.uploadSingleItem` but works entirely
/// with `Data` (images) or file `URL` (movies) already retrieved from the
/// extension item, rather than `PhotosPickerItem`.
///
/// Progress is reported via `onProgress` closure (value in `0.0 … 1.0`).
/// The caller should schedule UI updates on the main actor.
@MainActor
final class ShareUploadCoordinator: ObservableObject {
    ...
    func uploadImage(imageData: Data, mimeType: String, plotId: String, plotKey: SymmetricKey) async {
        await performUpload(plotId: plotId, plotKey: plotKey) {
    ...
    ///
    /// - Parameters:
    ///   - fileURL: Path to the video on disk (already copied to a temp file).
    ///   - mimeType: MIME type, e.g. `"video/mov"` or `"video/mp4"`.
    ///   - plotId: The target shared plot UUID.
    ///   - plotKey: 32-byte AES-256 plot key retrieved from the Keychain.
    func uploadVideo(fileURL: URL, mimeType: String, plotId: String, plotKey: SymmetricKey) async {
    ...
    private func performUpload(plotId: String, plotKey: SymmetricKey, work: @escaping () async throws -> Void) async {
        isUploading = true
        uploadError = nil
